...
interface AdminPanelProps {
  ...
  onClose: () => void;
  projects: Project[];
  onAddProject: (name: string, imageFile: File, link: string) => Promise<void>;
  onUpdateProject: (id: number, name: string, link: string, imageFile?: File) => Promise<void>;
      ...
      <div className="flex items-center space-x-4 overflow-hidden">
        ...
        <div className="overflow-hidden">
          ...
          <a 
            ...
            className="text-xs text-zinc-400 hover:text-blue-400 break-all truncate block"
          >
            {project.link}
          </a>
        </div>
...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Para novo projeto, a imagem é obrigatória
    ...
    try {
      if (editingProject) {
        ...
        // Criar novo projeto
        if (imageFile) {
          await onAddProject(formData.name, imageFile, formData.link);
        }
      }
    ...
    <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-lg w-full max-w-3xl max-h-[90vh] flex flex-col">
        <div className="p-4 border-b border-zinc-700 flex justify-between items-center">
          <h2 className="text-xl font-bold">Painel Admin - Gerenciar Projetos</h2>
          <button 
            onClick={onClose} 
        ...
        <div className="p-6 overflow-y-auto">
          <div id="project-form" className="bg-zinc-800 p-4 rounded-lg mb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
                  <div>
                    ...
                    >
                      {editingProject ? 'Atualizar Imagem (opcional)' : 'Upload de Imagem *'}
                    </label>
                    <input 
                      id="image-upload"
                      type="file" 
              ...
              <div className="flex space-x-4">
                ...
                >
                  {isUploading ? 'Salvando...' : (editingProject ? 'Atualizar Projeto' : 'Adicionar Projeto')}
                </button>
                {editingProject && (
                  <button 
                    type="button" 
          ...
          <div>
            <h3 className="font-bold mb-4 text-lg">Projetos Existentes</h3>
            {projects.length === 0 ? (
              <p className="text-zinc-400 text-center py-8">Nenhum projeto cadastrado ainda.</p>
            ) : (
